...
public class SteerableFilter3 {
  ...
  private static double[][] findCriticalPhiThetaNm(double[] fs) {
    return findCriticalPhiThetaNm(fs,null);
  }
  private static double[][] findCriticalPhiThetaNm(
    double[] fs, double[][] pts) 
  {
    double p0,p1,p2,t0,t1,t2,f0,f1,f2;
    if (pts!=null) {
      p0 = pts[0][0]; t0 = pts[0][1]; f0 = evals(fs,p0,t0);
      p1 =   p0+0.05; t1 = t0;        f1 = eval(fs,p1,t1);
      p2 =        p0; t2 = t0+0.05;   f2 = eval(fs,p2,t2);
    } else {
      double[] pti = initCriticalPhiTheta(fs);
      pts = new double[3][2];
      p0 = pti[0];   t0 = pti[1];   f0 = eval(fs,p0,t0);
      p1 = p0+0.3;   t1 = t0;       f1 = eval(fs,p1,t1);
      p2 = p1;       t2 = t0+0.3;   f2 = eval(fs,p2,t2);
    }

    // Iterate until maximum iterations or convergence.
    int maxiter = 50;
    boolean converged = false;
    double small = 0.01*PI;
    int niter,neval;
    for (niter=0,neval=0; niter<maxiter && !converged; ++niter) {

      // Order three points such that f0 <= f1 <= f2.
      if (f0>f1) {
        double pt = p0; p0 = p1; p1 = pt;
        double tt = t0; t0 = t1; t1 = tt;
        double ft = f0; f0 = f1; f1 = ft;
      }
      if (f1>f2) {
        double pt = p1; p1 = p2; p2 = pt;
      ...
      //System.out.println("p0="+p0+" t0="+t0+" f0="+f0);
      //System.out.println("p1="+p1+" t1="+t1+" f1="+f1);
      //System.out.println("p2="+p2+" t2="+t2+" f2="+f2);
      if (abs(p1-p0)<=small && abs(p2-p0)<=small && 
          abs(t1-t0)<=small && abs(t2-t0)<=small)
        break;

      // Reflected point, normalized to lie on sphere.
      double pm = 0.5*(p0+p1);
      double tm = 0.5*(t0+t1);
      double pr = 2.0*pm-p2;
      double tr = 2.0*tm-t2;
      ...
      double fr = evals(fs,pr,tr); ++neval;
      //System.out.println("pr="+pr+" tr="+tr+" fr="+fr);
      if (f0<=fr && fr<f1) {
        p2 = pr; t2 = tr; f2 = fr;
      ...
      // Else if new min, either expand or use reflected point to replace max.
        ...
        double pe = 2.0*pr-pm;
        double te = 2.0*tr-tm;
        double fe = evals(fs,pe,te); ++neval;
        if (fe<fr) {
          p2 = pe; t2 = te; f2 = fe;
      ...
      // Else if contraction (outside or inside) or shrink, ...
        ...
        if (fr<f2) {
          double pc = 0.5*(pr+pm);
          double tc = 0.5*(tr+tm);
          double fc = evals(fs,pc,tc); ++neval;
          if (fc<=fr) {
            p2 = pc; t2 = tc; f2 = fc;
        ...
        // If must shrink, ...
        if (shrink) {
          p1 = 0.5*(p0+p1); t1 = 0.5*(t0+t1); f1 = evals(fs,p1,t1); ++neval;
          p2 = 0.5*(p0+p2); t2 = 0.5*(t0+t2); f2 = evals(fs,p2,t2); ++neval;
          System.out.println("shrink");
        }
      }
    }
    System.out.println("niter="+niter+" neval="+neval+" p0="+p0+" t0="+t0);
    double p = modPi2(p0);
    double t = modPi(t0);
    pts[0][0] = p; pts[0][1] = t;
    pts[1][0] = p; pts[1][1] = t;
  ...
  private static double evals(double[] f, double p, double t) {
    double fe = eval(f,p,t);
    return fe;
  }
  public static double[] initCriticalPhiTheta(double[] f) {
    double f0 = f[0], f1 = f[1], f2 = f[2], f3 = f[3], f4 = f[4], f5 = f[5];
    double f0p1 = f0+f1, f2p3 = f2+f3, f4p5 = f4+f5;
    double fa = f0p1+f2p3-f4p5;
    double fb = f0p1+f4p5-f2p3;
    double fc = f2p3+f4p5-f0p1;
    double fm = f0;
    if (fm>f1) fm = f1;
    if (fm>f2) fm = f2;
    if (fm>f3) fm = f3;
  ...
  private static final double ORT2 = 1.0/sqrt(2.0);

  private static void initMinimumAbc(double[] f, double[] abc) {
    double f0 = f[0], f1 = f[1], f2 = f[2], f3 = f[3], f4 = f[4], f5 = f[5];
    double f0p1 = f0+f1, f2p3 = f2+f3, f4p5 = f4+f5;
    double fa = -(f4+f5);
    double fb = -(f2+f3);
  ...
  private static void initCriticalAbcX(double[] f, double[] abc) {
    double f0 = f[0], f1 = f[1], f2 = f[2], f3 = f[3], f4 = f[4], f5 = f[5];
  ...
  private static void initExtremeAbc(
    double fab, double fac, double fbc,
    double faa, double fbb, double fcc,
    double[] abc) 
  {
  ...
  private static void findCriticalAbc(double[] f, double[] abcf) {
    ...
    // Initial the three (a,b,c).
    initExtremeAbc(fab,fac,fbc,faa,fbb,fcc,abcf);
    double a = abcf[0], b = abcf[1], c = abcf[2];
    double p = modPi2(atan2(b,a));
    double t = acos(c);
    System.out.println("init: phi="+p+" theta="+t);
    ...
    for (niter=0; niter<50; ++niter) {
      ...
      p = modPi2(atan2(b,a)); t = acos(c);
      //System.out.println("niter="+niter+" phi="+p+" theta="+t);
    ...
    System.out.println("niter="+niter+" fabc="+fabc);
    if (niter>10)
      System.out.println(
        "*******************************************************************");
   ...
   * where (a,b,c) is a point on the unit-sphere.
   */
  private static void findMinimumAbc(double[] f, double[] abcf) {
