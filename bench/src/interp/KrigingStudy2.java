...
public class KrigingStudy2 {
  public static void compare12(
    int n, int nx, int ny,
    double sigmaM, double shapeM, double rangeM, double sigmaD) 
  {
    ...
    DMatrix cm = covM(sigmaM,shapeM,rangeM,sx,sy);
  ...
  public static DMatrix covM(
    double sigma, double shape, double range, Sampling sx, Sampling sy)
  {
    MaternCovariance cm = new MaternCovariance(sigma,shape,range);
    int nx = sx.getCount();
    int ny = sy.getCount();
    int m = nx*ny;
  ...
  {
    return gridKriging(sigma,1.0f,range,f,x,y,sx,sy);
  }

  public static float[][] gridKriging(
    float sigma, float shape, float range,
    float[] f, float[] x, float[] y, Sampling sx, Sampling sy)
  {
    MaternCovariance cm = new MaternCovariance(sigma,shape,range);
    int n = f.length;
  ...
  public static void go() {
    ...
    compare12(n,nx,ny,sigmaM,shapeM,rangeM,sigmaD);
  }
}
