...
public class WellLogWarping4 {
  ...
  public float[][] findShifts(float[][] fs) {
    ...
    // Compute weights and interpolate/extrapolate pairs.
    computeWeights(fs,ps);
    ps = extendPairs(nz,ps);
    ...
    // Array for strains q at time zero to be added to CG solution. 
    double[] q0 = new double[nl];

    // Array subtracted from right-hand-side b before CG iterations.
    double[][] b0 = new double[nl][nt];
    VecArrayDouble2 vb0 = new VecArrayDouble2(b0);

    // Outer iterations over CG solutions, which depend on t[il][iz].
    int maxouter = 5; // is this number sufficient? more than necessary?
    for (int nouter=0; nouter<maxouter; ++nouter) {

      // Inner preconditioned CG iterations.
      int ninner = 50+50*nouter;
      ...
      if (nouter<maxouter-1) {
        ...
        // Update q0 and b0 to be subtracted from b in next outer iteration.
        zero(b0);
        for (int il=0; il<nl; ++il)
          b0[il][0] = q0[il] = sum(r[il])/nt;
        dump(q0);
        a.apply(vb0,vb0);
      }
    }
