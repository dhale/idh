...
datadir = "/data/seis/cvx/"
True = 1
False = 0

# 
n1i=151 # number of time samples (1st dimension)
d1i=0.004 # time sampling interval
f1i=0.000 # time of first sample
n2i=5743 # number of traces in 2nd dimension
d2i=0.005
f2i=0.000
n3i=247 # number of traces in 3rd dimension
d3i=0.005
f3i=0.000

# sampling after windowing
n1,n2,n3 = n1i,247,n3i
...
def main(args):
  #readTracesNative("suprjarm1.H@",n1i,n2i,n3i)
  #readTraces("x1.dat",n1,n2,n3)
  #window()
  x = readFile("x1.dat",n1,n2,n3)
  plot3d(x)
  return

def readTraces(file,n1,n2,n3):
  ...
  for i3 in range(n3):
    for i2 in range(n2):
      ...
      xmin = Array.min(x)
      xmax = Array.max(x)
      print "readTraces: i2 =",i2,"i3 =",i3,"min =",xmin," max =",xmax

def readTracesNative(file,n1,n2,n3):
  ais = ArrayInputStream(datadir+file,ByteOrder.LITTLE_ENDIAN)
  x = Array.zerofloat(n1)
  for i3 in range(n3):
    for i2 in range(n2):
      ais.readFloats(x)
      xmin = Array.min(x)
      xmax = Array.max(x)
      if i2>=3500 and i2<3747 and xmin<32000 and xmax>32000:
        print "readTraces: i2 =",i2,"i3 =",i3,"min =",xmin," max =",xmax

def readFileNative(file,n1,n2,n3):
  ais = ArrayInputStream(datadir+file,ByteOrder.LITTLE_ENDIAN)
  x = Array.zerofloat(n1,n2,n3)
  ais.readFloats(x)
  print "readFileNative: x min =",Array.min(x)," max =",Array.max(x)
  ais.close()
  return x
...
def window():
  j1,j2,j3 =  0,3500,0
  x = readFileNative("suprjarm1.H@",n1i,n2i,n3i)
  y = Array.copy(n1,n2,n3,j1,j2,j3,x)
  print "x min =",Array.min(x)," max =",Array.max(x)
  print "y min =",Array.min(y)," max =",Array.max(y)
  writeFile("x1.dat",y)

def plot3d(x,cmin=-10000.0,cmax=10000):
  print "x min =",Array.min(x)," max =",Array.max(x)
  s1 = Sampling(n1,d1,f1)
  s2 = Sampling(n2,d2,f2)
  s3 = Sampling(n3,d3,f3)
